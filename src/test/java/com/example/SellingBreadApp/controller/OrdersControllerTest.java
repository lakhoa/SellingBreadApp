...
class OrdersControllerTest {
    ...
    @Autowired
    private OrdersService ordersService; // TODO: Unused dependency???
    @Autowired
    ...
    @Autowired
    private IngredientController ingredientController; // TODO: Unused dependency???
    ...
    private String createRequestDTO(
            List<Product> productList, Long productId, Long toppingId, Integer quantityTopping)
        ...
        OrderItemDetailRequestDTO orderItemDetailRequestDTO1 =
                new OrderItemDetailRequestDTO(topping1.getId(), 1);
        OrderItemDetailRequestDTO orderItemDetailRequestDTO2 =
                new OrderItemDetailRequestDTO(toppingId, quantityTopping);
        OrderItemDetailRequestDTO orderItemDetailRequestDTO3 =
                new OrderItemDetailRequestDTO(topping2.getId(), 1);
    ...
    public void shouldReturnBillWhenCreateOrderRight() throws Exception {
        ...
        Product product1 = productList.get(0);
        Product product2 = productList.get(1); // TODO: Unused variable?
        Topping topping3 = product1.getToppings().get(2);
        Topping topping2 = product1.getToppings().get(1); // TODO: Unused variable?
        String requestDTO = createRequestDTO(productList, product1.getId(), topping3.getId(), 1);
        mockMvc.perform(post("/api/v1/order")
                        .contentType(APPLICATION_JSON_UTF8)
                        .content(requestDTO))
                        .andDo(MockMvcResultHandlers.print()) // TODO: Pls remove this one when you push code
                        .andExpect((status().isOk()))
                        .andExpect(
                            jsonPath("$.data.orderItemResponseDTOList[:1].productName").value(product1.getName()))
                        .andExpect(
                            jsonPath("$.data.orderItemResponseDTOList[:1].productPriceUnit")
                                        .value(product1.getPrice()))
                        .andExpect(
                            jsonPath(
                            "$.data.orderItemResponseDTOList[1].orderItemDetailResponseDTOList[1].toppingName")
                                        .value(topping3.getName()))
                        .andExpect(jsonPath("$.data.totalPrice").value(295.0));

        PageRequest.of(0, 1); // TODO: What is the purpose of this line?
